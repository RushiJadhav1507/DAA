...
void lps_func(string txt, int Lps[])
{
    Lps[0] = 0;                   
    int len = 0;
    int i=1;
...
void KMP(string pattern,string text){
    ...
    while(i<n)
	{
        ...
        if (j == m) 
		{ 
            cout<<"Pattern is matched at index:"<<i - m <<' '<<endl;    // if j==m it is confirmed that we have found the pattern and we output the index
